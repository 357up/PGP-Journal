...
class Login
{
    ...
    public function __construct()
    {
        ...
        // if user has an active session on the server
            ...
            } elseif (isset($_POST["user_edit_submit_pgp"])) {
                // function below uses $_SESSION['user_name'] and $_SESSION['user_id']
                $this->editUserPGP($_POST['user_pgp']);
            } elseif (isset($_POST["newEntry_form_submit"])) {
                // function below uses $_SESSION['user_name'] and $_SESSION['user_id']
                $this->saveNewEntry($_POST['title'],$_POST['date'],$_POST['entry']);
            } elseif (isset($_POST["newEntry_form_update_submit"])) {
                // function below uses $_SESSION['user_name'] and $_SESSION['user_id']
                $this->saveUpdateEntry($_POST['title'],$_POST['date'],$_POST['entry'],$_POST['recordno']);
            }
    ...
    /**
     * Edit the user's Public PGP Key, provided in the editing form
     */
    public function editUserPGP($user_pgp)
    {
        if (empty($user_pgp)) {
            $this->errors[] = "PGP field is empty.";
        } else if ($this->databaseConnection()) {
            // write user's new data into database
            $query_edit_user_pgp = $this->db_connection->prepare('UPDATE users SET user_pgp = :user_pgp WHERE user_id = :user_id');
            $query_edit_user_pgp->bindValue(':user_pgp', $user_pgp, PDO::PARAM_STR);
            $query_edit_user_pgp->bindValue(':user_id', $_SESSION['user_id'], PDO::PARAM_INT);
            $query_edit_user_pgp->execute();

            if ($query_edit_user_pgp->rowCount()) {
                $this->messages[] = "Public PGP key changed successfully.";
            } else {
                $this->errors[] = "Public PGP key change failed.";
            }
        }
    }
     ...
     * Save the new entry
     */
    public function saveNewEntry($title,$date,$entry)
    {
        if (empty($title) || empty($date) || empty($entry)) {
            $this->errors[] = "Empty Fields. Unable to Save.";
        } else if ($this->databaseConnection()) {
            // write user's new data into database
            $query_edit_user_pgp = $this->db_connection->prepare('INSERT INTO journal_entries(`Title`,`Date`,`Entry`,`user_id`) VALUES(:title,:datesaved,:entry,:user_id)');
            $query_edit_user_pgp->bindValue(':title', $title, PDO::PARAM_STR);
            $query_edit_user_pgp->bindValue(':datesaved', $date, PDO::PARAM_STR);
            $query_edit_user_pgp->bindValue(':entry', $entry, PDO::PARAM_STR);
            $query_edit_user_pgp->bindValue(':user_id', $_SESSION['user_id'], PDO::PARAM_INT);
            $query_edit_user_pgp->execute();

            if ($query_edit_user_pgp->rowCount()) {
                $this->messages[] = "Entry saved successfully.";
            } else {
                $this->errors[] = "Error on save. Entry was not saved.";
            }
        }
    }
     ...
     * Update the entry
     */
    public function saveUpdateEntry($title,$date,$entry,$recordNo)
    {
        if (empty($title) || empty($date) || empty($entry) || empty($recordNo)) {
            $this->errors[] = "Empty Fields. Unable to Save.";
        } else if ($this->databaseConnection()) {
            // write user's new data into database
            $query_edit_user_pgp = $this->db_connection->prepare('UPDATE journal_entries SET `Title` = :title,`Date` = :datesaved,`Entry` = :entry WHERE user_id = :user_id AND RecordNo = :recordno');
            $query_edit_user_pgp->bindValue(':title', $title, PDO::PARAM_STR);
            $query_edit_user_pgp->bindValue(':datesaved', $date, PDO::PARAM_STR);
            $query_edit_user_pgp->bindValue(':entry', $entry, PDO::PARAM_STR);
            $query_edit_user_pgp->bindValue(':user_id', $_SESSION['user_id'], PDO::PARAM_INT);
            $query_edit_user_pgp->bindValue(':recordno', $recordNo, PDO::PARAM_INT);
            $query_edit_user_pgp->execute();

            if ($query_edit_user_pgp->rowCount()) {
                $this->messages[] = "Entry updated successfully.";
            } else {
                $this->errors[] = "Error on update. Entry was not saved.";
            }
        }
    }
